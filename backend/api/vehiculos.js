...
endpointsVehiculos.get("/", validarFiltrosVehiculo, async (req, res) => {
    try {
        const texto = "SELECT v.id, v.nombre, v.tipo, c.nombre as ubicacion, v.motor, v.estructura, v.combustible, v.punto_interes FROM vehiculos as v, cuerpos_celestes as c WHERE c.id=v.ubicacion_id AND v.borrado = FALSE AND c.borrado = FALSE";
