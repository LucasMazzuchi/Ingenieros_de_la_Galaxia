...
endpointsVehiculos.get("/", validarFiltrosVehiculo, async (req, res) => {
    try {
        const texto = "SELECT v.id, v.nombre, v.tipo, c.nombre as ubicacion, v.motor, v.estructura, v.combustible FROM Vehiculos as v, CuerposCelestes as c WHERE c.id=v.ubicacionId AND v.borrado = FALSE AND c.borrado = FALSE";
