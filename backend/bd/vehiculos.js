...
export async function getVehiculo(id) {
    const solicitud = "SELECT v.id, v.nombre, c.nombre as ubicacion, v.tipo, v.motor, v.estructura, v.combustible, v.punto_interes FROM vehiculos as v, cuerpos_celestes as c WHERE v.id=$1 AND c.id=v.ubicacion_id AND v.borrado = FALSE AND c.borrado = FALSE";
