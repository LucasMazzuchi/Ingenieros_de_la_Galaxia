...
export async function getAllVehiculos(vehiculo, entidad ) {
    const sinFiltro = "SELECT v.id, v.nombre, v.motor, v.estructura, v.resistencia, v.combustible, v.punto_interes, v.ubicacion_id FROM vehiculos as v WHERE v.borrado = FALSE";
...
export async function getVehiculo(id) {
    const solicitud = "SELECT v.id, v.nombre, v.motor, v.estructura, v.resistencia, v.combustible, v.punto_interes, v.ubicacion_id FROM vehiculos as v WHERE v.id=$1 AND v.borrado = FALSE";
