...
export async function getMision(id) {
    const solicitud = "SELECT m.id, m.nombre, c.nombre AS cuerpo_celeste, m.descripcion, m.relevancia, m.porcentaje, m.disponible, m.imagen FROM Misiones as m, CuerposCelestes as c WHERE m.id = $1 AND c.id = m.cuerpoCelesteId AND m.borrado = FALSE AND c.borrado = FALSE";
...
export async function removeMision(id){
    const solicitud = " Update Misiones SET borrado = TRUE WHERE id=$1";
    const res = await db.query(solicitud, [id]);
    return res.rowCount == 1;
}

export async function updateMision(mision){
    const { consulta, valores } = armar_consulta(mision)
