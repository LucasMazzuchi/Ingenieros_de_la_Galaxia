...
async function iniciarPlaneta() {
    ...
    try {
        ...
        const {tituloCompletado, textoCompletado} = await estaCompletado(estado, puntosInteres, naveId, planetaId);
...
async function manejarClickPunto(cuerpoCelesteId, puntoInteres, vehiculoId, coordenadasDestino, vehiculoDOM) {
    ...
    if (estamosAhi) {
        try {
            const estadoPlanetaAntes = await planetaCompletado(cuerpoCelesteId, vehiculoId);
...
function crearTransicion(posNave) {
...
async function estaCompletado(estado, puntosInteres, naveId, planetaId){
