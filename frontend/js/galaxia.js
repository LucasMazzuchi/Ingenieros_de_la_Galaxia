...
async function pintarPlanetas(cuerpos_celestes, naveId) {
  ...
  pintarEstacionEspacial(estadoTierra);
  contenedor.innerHTML = "";
  let completados = 0;
  for (const cuerpo of cuerpos_celestes){
    const div = armarDivCuerpoCeleste(cuerpo, estadoTierra);
...
// La función pinta la estación espacial e inicializa el evento que debe ocurrir al tocarla. Si estadoTierra.planetaCompletado es true, te redirige a
// estacion_espacial.html, sino te muestra por pantalla que no está disponible y se la ve con una capa oscura y un texto encima.
function pintarEstacionEspacial (estadoTierra) {
...
// La función arma el div para el cuerpo pasado por parámetro, inicializa la redirección a su url en caso de estar disponible y lo devuelve. Si el cuerpo no está
// disponible, se agrega una capa oscura por encima con una leyenda que indica como desbloquearlo. Además, si se toca el planeta, se muestra por pantalla
// con más detalle que hay que hacer.

function armarDivCuerpoCeleste (cuerpo, estadoTierra) {
